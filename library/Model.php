...
abstract class Model
{
    ...
    public function setAttr($name, $value, $data = [])
    {
        if (is_null($value) && $this->autoWriteTimestamp && in_array($name, [$this->createTime, $this->updateTime])) {
            ...
            // 检测修改器
            $method = 'set' . $this->parseName($name) . 'Attr';
    ...
    public function getAttr($name)
    {
        ...
        // 检测属性获取器
        $method = 'get' . $this->parseName($name) . 'Attr';
        if (method_exists($this, $method)) {
            ...
            $method = $this->parseName($name, 1, false);
     ...
     * 转换当前模型数据集为数据集对象
     * @access public
     * @param array| $collection 数据集
     * @return Collection
     */
    public function toCollection($collection)
    {
        if (!empty($this->resultSetType)) {
     ...
     * @return array
     */
    public static function pageList($where=[], $page=1, $count=10, $order='')
    {
        $data = [];
        $data['list'] = self::where($where)->page($page, $count)->order($order)->asArray()->select();
        $data['count'] = self::where($where)->count(1);
        return $data;
    }
     ...
     * @return mixed
     */
    public static function findOne($where=[], $fields=[], $cache=false)
    {
        if (is_int($where) || (!is_array($where) && is_numeric($where)))
        {
            return self::field($fields)->cache($cache)->asArray()->find(intval($where));
        }
        return self::field($fields)->where($where)->cache($cache)->asArray()->find();
    }
     ...
     * @return mixed
     */
    public static function findAll($where=[], $fields=[], $cache=false)
    {
        return self::field($fields)->where($where)->cache($cache)->asArray()->select();
    }
     ...
     * @throws Exception
     */
    public static function getSelect($where=[], Array $fields=[])
    {
        // 判断是否有两个字段
        if (count($fields) < 2) {
            return '$fields need least two';
        }
        // 获取所有内容
        $temp = self::field($fields)->where($where)->asArray()->select();
        // 获取键1和值1
     ...
     * @return Query
     */
    protected static function parseQuery(&$data, $with, $cache=false)
    {
    ...
    public static function destroy($data)
    {
        ...
        if (is_numeric($data)){
            ...
            // 若主键为空，则不删除
            if (empty($pk)) {
                throw new \Exception('There is not pk in table');
            }
            // 若为字符串，则只有一个主键
            if (is_string($pk)) {
                $where[$pk] = $data;
        ...
        return self::whereOr($where)->delete();
    }
    ...
    protected function parseModel($model)
    {
        if (false === strpos($model, '\\')) {
            ...
            array_pop($path);
            array_push($path, $this->parseName($model, 1));
            $model = implode('\\', $path);
        }
        return $model;
    }
    ...
    public function relationQuery($relations)
    {
        ...
        foreach ($relations as $key => $relation) {
            ...
            $method                = $this->parseName($relation, 1, false);
    ...
    public function eagerlyResultSet(&$resultSet, $relation)
    {
        ...
        foreach ($relations as $key => $relation) {
            ...
            $relation = $this->parseName($relation, 1, false);
    ...
    public function relationCount(&$result, $relation)
    {
        ...
        foreach ($relations as $key => $relation) {
            ...
            if (!isset($name)) {
                $name = $this->parseName($relation) . '_count';
            }
    ...
    protected function getForeignKey($name)
    {
        ...
        return $this->parseName($name) . '_id';
    }
    ...
    public function belongsToMany($model, $table = '', $foreignKey = '', $localKey = '')
    {
        // 记录当前关联信息
        $model      = $this->parseModel($model);
        $name       = $this->parseName(basename(str_replace('\\', '/', $model)));
        $table      = $table ?: $this->db(false)->getTable($this->parseName($this->name) . '_' . $name);
    ...
    public function morphMany($model, $morph = null, $type = '')
    {
        ...
        if (is_null($morph)) {
            ...
            $morph = $this->parseName($trace[1]['function']);
        }
        $type = $type ?: $this->parseName($this->name);
        if (is_array($morph)) {
