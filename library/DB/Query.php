...
class Query
{
    ...
    public function parseSqlTable($sql)
    {
        ...
        return $sql;
        /**
        // 判断是否有前缀，并判断名称上是否已加前缀
        if (!empty($this->prefix)
            && strpos($tableName, $this->prefix) !== 0) {
    ...
    {
        empty($field) && $field = '*';
        return intval($this->value('COUNT(' . $field . ') AS `tp_count`', 0, true));
    }
    ...
    {
        $this->options['group'] = is_string($group) ? explode(',', $group) : $group;
        return $this;
    }
...
//            }
    ...
    public function withCount($relation, $subQuery = true)
    {
        if (!$subQuery) {
            ...
            foreach ($relations as $key => $relation) {
                ...
                $this->field([$count => $this->parseName($relation) . '_count']);
            }
        }
        return $this;
    }
    ...
    {
        if (!empty($this->model)) {
            throw new \Exception('model data Not Found:' . $this->model);
        } else {
            $table = is_array($options['table']) ? key($options['table']) : $options['table'];
