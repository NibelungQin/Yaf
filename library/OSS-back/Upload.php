...
class OSS_Upload
{
     ...
     * @return none
     */
    public static function file($folder='', $bucket='')
    {
        // 文件夹处理
    ...
    public static function base64_img($base64='', $folder = 'base64', $bucket='')
    {
        ...
        {
            // 获取OSS客户端对象
            $OssClient = OSS_Common::getOssClient($bucket);
            // 删除左侧的/
            ...
            return [
                'status' => true,
                'info' => '文件保存成功',
                'path' => OSS_Common::get_httpUrl($bucket).$path,
            ];
        }
        else
        {
    ...
    private static function oss($files=[], $folder='', $bucket='')
    {
        ...
        foreach ($files as $key => &$file)
        {
            ...
            unset($file['tmp_name']);
            // 获取OSS访问路径
            $file['path'] = OSS_Common::get_httpUrl($bucket).$folder.$name;
            // 保存至数据库
            self::saveFile($file);
        }
    ...
    private static function saveFile(&$file=[])
    {
        ...
        $file['user_id'] = self::$user['user_id'];

        $file['user_type'] = self::$user['user_type'];

        $temp = Sys_FileModel::create($file);
    ...
    private static function hashCheck(&$files=[])
    {
        ...
        // 按照hash查找数据库
        $data = Sys_FileModel::have_files($hashs);
        // 若数据库中没有重复的内容
        if (empty($data))
        {
            return [];
