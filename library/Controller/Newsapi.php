...
class Newsapi extends \Yaf_Controller_Abstract
{
    // 后台用户编号
    protected $admin_id;
    // 后台客户组编号
...
//            ['appid', 'require', '缺少应用ID'],
        ...
        // 判断解码是否成功
        empty($auth) && $this->errorCode(10000, '请勿非法访问', 'decode error');
        // 进行json解码
        $auth = json_decode($auth, true);
        // 判断解码是否成功
        empty($auth) && $this->errorCode(10000, '请勿非法访问', 'json_decode error');
        // 进行全局赋值
        $this->auth = $auth;
    }
