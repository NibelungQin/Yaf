...
class Allxcx extends \Yaf_Controller_Abstract
{
    // 后台用户编号
    protected $admin_id;
    // 后台客户组编号
    ...
    {
        $res = \Encrypt::undes($param);
        $res = json_decode($res,true);
        $res = intval($res);
        if (empty($res)) 
        {
            $this->ajaxReturn(false,1002,'验证失败');
        }
        else
        {
            return $res;
     ...
     * 在每次控制器方法调用 完成后需要执行
     */
    public function setNewToken($param)
    {
    ...
    */
    public function checkToken($token='')
    {
        if($token)
        {
            ...
            if($claims)
            {
                $this->aid    = isset($claims['param']['aid'])? $claims['param']['aid']:0;
                $this->openid = isset($claims['param']['openid'])? $claims['param']['openid']:0;
                $this->auth['openid'] = isset($claims['param']['openid'])? $claims['param']['openid']:0;
    ...
    protected function getParams(Array $rule=[])
    {
        ...
        if ($result == false)
        {
            $this->ajaxReturn(false,1001, $Validate->getError());
        }
        return $data;
    }
